...
class DocumentAnalyzer {
  ...
  _calculateReadability(document) {
    ...
    const readingEase = Math.max(0, Math.min(100, 206.835 - 1.015 * (words / sentences) - 84.6 * (syllables / words)));

    return {
      score: Math.round(readingEase),
      difficulty: readingEase > 80 ? 'Easy' : readingEase > 50 ? 'Medium' : readingEase > 30 ? 'Difficult' : 'Very Difficult',
      wordCount: words,
  ...
  _findDifferences(doc1, doc2) {
    ...
    return {
      ...
      wordCountDifference: Math.abs(text1.split(/\s+/).length - text2.split(/\s+/).length),
    };
  }
